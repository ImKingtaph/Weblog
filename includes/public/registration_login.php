<?php include(BASE_URL. 'config.php'); ?>
<?php
...
if (isset($_POST['register_btn'])) {
    ...
    if (empty($errors)) {
        $password = md5($password); // encrypt password
        $sql = "INSERT INTO users (username, email, role, password) VALUES ('$username', '$email', 'Author', '$password')";
        $result = mysqli_query($conn, $sql);
        //vardummp result
        //die
...
function getUserById($id)
{
    ...
    $sql = "SELECT users.username, users.role FROM users WHERE users.id='$id'";// requête qui récupère le user et son rôle
